...
::       source audio into MP4 fails for non-MP4-legal codecs (PCM/Opus/Vorbis).
...
rem --- Audio ---
rem   aac  : re-encode to AAC (safe for MP4; default).
rem   copy : stream-copy source audio (only if MP4-legal in ALL inputs).
set "AUDIO_MODE=aac"
    ...
    if !DONE! GTR 0 (
        if exist "%CONCAT_LIST%" (
            ...
            if /I "%CONCAT_MODE%"=="auto" (
                rem AV1-in-MP4 copy-concat is brittle; re-encode under auto.
